...
import { Check, Eye, PenTool, X } from "react-feather";
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (t: string, record: any) => {
        return (
          <Tooltip label="Кўриш">
            <Link href={`/control/regions/${record?.id}`}>
    ...
    },
    {
      ...
      dataIndex: "title",
      key: "title",
      render: (t: string) => t[0].toString().toUpperCase() + t.slice(1),
    },
    {
      title: "Туманлар сони",
      dataIndex: "districts",
  ...
  return (
    ...
    >
      ...
      <PaperContent>
        ...
        <FilterControlpage />
        <Box>
          <TableGen columns={columns} dataSource={regions} />
          <Pagination
            total={total}
            current={current}
            pageSize={pageSize}
